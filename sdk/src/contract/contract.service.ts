...
export interface UnsignedTxResult<T = any> {
  ...
  simulatedResult: ContractResponse<T>;
...
export class ContractService {
  ...
  async simulateReadOnly<T>(method: ContractFnName | string, params: any[]): Promise<ContractResponse<T>> {
  ...
  async invoke<T = any>(
    ...
    try {
      ...
      if (options.simulateOnly) {
        return { success: true, value: sim.returnValue as T, txHash: '', transactionHash: '', ledger: 0 };
      }
      ...
      return {
        ...
        transactionHash: polled.txHash, // Deprecated: use txHash
        ledger: polled.ledger,
        status: 'SUCCESS',
        feeCharged: polled.feeCharged,
   ...
   */
  async submitSigned<T = any>(signedXdr: string): Promise<ContractResponse<T>> {
    if (!signedXdr) {
      throw new TikkaSdkError(
        TikkaSdkErrorCode.InvalidParams,
